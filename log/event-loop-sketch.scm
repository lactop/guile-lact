...
; Да, я переписал это из книжки и не очень хорошо понимаю. Нужны пояснения.
; Видимо, (succ b Γ fail) означает: мы подобрали значение b запрошенной формулы
; в окружении Γ; если оно не подходит для вас (на текущем уровне по структуре
; формулы), вызовете fail, и мы тебе подберём другое значение, пройдя по всем
; нижележащим уровням структуры формулы.

; По формуле можно сразу построить структуру перебора окружений Γ и дерева точек
; отката fail. Для этого можно использовать каррирование. Из общего списка
; параметров для succ мы выносим fail и Γ, именно они меняются во всём этом
; переборе.
